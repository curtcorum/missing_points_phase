...
%
%  Modified by Curt Corum to only make hdr, 10/17/2014
%     hdr = make_nii_hdr(imgsize, [], [], datatype, maxval, minval);
function hdr = make_nii_hdr(varargin)
...
if nargin > 1 & ~isempty(varargin{2})
    voxel_size_in = double(varargin{2});
    sz_voxel_size_in = size( voxel_size_in);
    sz_voxel_size_in = sz_voxel_size_in(2);
    if sz_voxel_size_in == 3
        voxel_size(2:4) = double(varargin{2});
    elseif sz_voxel_size_in == 4
...
hdr = make_header(dims, voxel_size, origin, datatype, ...
